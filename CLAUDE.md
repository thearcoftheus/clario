...
- **Feedback reports + portal (first deploy only):**
  ...
  - ⚠️ **The database is SQLite** (`DB_CONNECTION=sqlite`). Confirm the database file lives on persistent, backed-up storage on Cloudways before the cohort starts sending reports — a deploy that wipes it takes the feedback with it.
